...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_group_admin(update, context):
        ...
        if g["channel"] and not await is_subscribed(update.effective_user.id, g["channel"], context):
            await update.message.reply_text(f"⚠️ لازم تشترك بـ {g['channel']} اول", reply_markup=sub_keyboard(g["channel"]))
        else:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_group_admin(update, context):
        return
    if query.data == "stats":
        await query.edit_message_text(f"📊 احصائيات القروب:\n\n👥 المستخدمين: {len(g['users'])}\n📢 القناة: {g['channel'] or 'ما متحددة'}\n👑 المطور: @ZERO_MAX_COOL", reply_markup=admin_panel())
...
async def handle_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_group_admin(update, context):
        if g["channel"] and not await is_subscribed(user_id, g["channel"], context):
            await update.message.reply_text(f"⚠️ اشترك بـ {g['channel']} اول", reply_markup=sub_keyboard(g["channel"]))
            try:
